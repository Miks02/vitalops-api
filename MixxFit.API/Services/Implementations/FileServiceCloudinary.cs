    ...
    public class CloudinaryFileService : IFileService
    {
        ...
        public async Task<Result<string>> UploadFile(IFormFile file, string? uploadedFilePath, string? uploadSubDir)
        {
            ...
            if (uploadResult.Error != null)
            {
                ...
                return Result<string>.Failure(Results.Error.File.ValidationFailed());
            }
        ...
        private Result IsFileValid(IFormFile file)
        {
            ...
            if (fileSize == 0)
                return Result.Failure(Results.Error.File.Empty());

            var maxFileSize = 5 * 1024 * 1024;

            if (fileSize > maxFileSize)
                return Result.Failure(Results.Error.File.TooLarge(maxFileSize));
            ...
            if (!allowedExtensions.Contains(extension))
                return Result.Failure(Results.Error.File.UnsupportedExtension(extension));
            return Result.Success();
        }
